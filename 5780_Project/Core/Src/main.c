...
// vars below used in calculation of the PID terms
int Kp = 75;
int Ki = 16;
int Kd = 10;
int F_sample = 50; // sample rate of pixy
float Delta_t =
    1.0 / 50; // value used for approximation of time between samples

// used for storing the calculated previous integral
float PID_Ix;
float PID_Iy;

uint8_t phase;
int pan = 2240, tilt = 3600; // representing 0 for the servos
...
int main(void) {
  ...
  // added startup delay to prevent reset faults on the Pixy due to brownouts
  HAL_Delay(1000);
  PWM_init();
  ...
  GPIOC->AFR[0] |=
      (1 << 16) | (1 << 20);          // Set to correct AF mode for pin 4 and 5
...
/*
 * All interrupts use the same interrupt handler. Must check status of flags
...
void USART3_4_IRQHandler() {
  ...
  case 2:
    ...
    break;
  case 8:
    // The x center has been receieved, update values of x for (t-1) and (t-2)
    prev2_x = prev_x;
    prev_x = curr_x;
    curr_x = block;
    phase++;
    break;
  case 10:
    // The y center has been received, update values of y for (t-1) and (t-2)
    prev2_y = prev_y;
    prev_y = curr_y;
    curr_y = block;
    phase++;
...
void track_object(void) {
  // minimization of work by re-using local values.
  uint16_t x = (curr_x - 157);
  uint16_t mx = 157 - curr_x;
  uint16_t y = curr_y - 103;
  uint16_t my = 103 - curr_y;

  // Single time delay calculation for derivative control
  // represents K*(e(t)-e(t-1))
  float PID_Dx = (Kd / Delta_t) * (curr_x - prev_x);
  float PID_Dy = (Kd / Delta_t) * (curr_y - prev_y);

  // integral Ki*( Delta_t/2 * [e(t) + 2*e(t-1) +2e(t-2) ... + e(t-n)] )
  // Tot_int = Current_int
  // clamp to 1000
  PID_Ix += Ki * (Delta_t / 2 * (curr_x + 2 * prev_x + prev2_x));
  PID_Iy += Ki * (Delta_t / 2 * (curr_y + 2 * prev_y + prev2_y));
  ...
  float valx = Kp * (x) + PID_Ix + PID_Dx;
  float valy = Kp * (y) + PID_Iy + PID_Dy;

  if (x > 10 || mx > 10 || y > 10 || my > 10) {
    //
    // pan = pan - (curr_x - 157);
    // tilt = tilt + (curr_y - 103);
    pan = (uint16_t)valx;
    tilt = (uint16_t)valy;
    if (tilt < 900)
      tilt = 900;
...
void PWM_init(void) {
  ...
  TIM3->CCMR1 |= (6 << 4);  // Set channel 1 to PWM mode 1 - In upcounting,
                            // channel 1 is active when TIMx_CNT < TIMx_CCR1
  TIM3->CCMR1 |= (6 << 12); // Set channel 2 to PWM mode 1 - In upcounting,
                            // channel 2 is active when TIMx_CNT < TIMx_CCR2

  // Output compare 1 and 2 preload enable. Read/Write operations go to preload
  // register, preload value loaded in active register at update events.
 ...
 * @param LED_num (int): The number corresponding to the LED pin. Ex: PC6 would
 * be 6.
 * @retval int: 1 if LED is high, 0 otherwise
 */
int LED_is_on(int LED_num) { return GPIOC->ODR & (1 << LED_num); }
 ...
 * @retval None
 */
void turn_on_LED(int LED_num) { GPIOC->ODR |= (1 << LED_num); }
 ...
 * @retval None
 */
void turn_off_LED(int LED_num) { GPIOC->ODR &= ~(1 << LED_num); }
